...
class Ukraine:
    ...
    def _load_dose_data(self, dose_param, colname):
        doses_api = requests.get(
            f"{self.source_api_url}?dose={dose_param}"
        ).json()
    ...
    def pipeline_manufacturer(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for vaccine, col in zip(vaccine_name, column_name):
            ...
            vac_dfs.append(
                vac_df[["location", "date", "vaccine", "total_vaccinations"]]
            )

        return (
            pd.concat(vac_dfs, ignore_index=True).sort_values(["date", "vaccine"])
        )
